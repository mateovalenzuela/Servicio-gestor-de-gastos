...
namespace BackendGastos.Validator.Ingreso
{
    public class InsertUpdateIngresoValidator : AbstractValidator<InsertUpdateIngresoDto>
    {
        public InsertUpdateIngresoValidator()
        {
            ...
            RuleFor(i => i.Descripcion).Length(2, 30).WithMessage("La {PropertyName} es obligatoria y debe tener de 2 a 30 caracteres");

            //RuleFor(i => i.Importe).NotEmpty().WithMessage("La {PropertyName} es obligatorio");
            RuleFor(i => i.Importe).PrecisionScale(18, 2, false).WithMessage("El {PropertyName} es obligatorio y debe tener un formato de (18,2)");
